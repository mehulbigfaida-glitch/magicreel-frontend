...
export default function ReelOutputPage() {
  ...
  return <div className="carousel-reel-page"><div className="carousel-reel-orb carousel-reel-orb-one"/><div className="carousel-reel-orb carousel-reel-orb-two"/><main className="carousel-reel-shell">
    ...
    <section className="carousel-reel-hero">
      <aside className="carousel-reel-left"><div className="carousel-reel-kicker">MAGICREEL AI STUDIO</div><h1>Carousel Reel</h1><p className="carousel-reel-description">A cinematic showcase of your collection with smooth transitions and elegant camera motion.</p><div className="carousel-reel-scenes">{scenes.slice(0,6).map((scene,index)=><button key={`${scene.poseId}-${index}`} className={`carousel-reel-scene ${selectedScene===index?"selected":""}`} onClick={()=>setSelectedScene(index)}><img src={scene.imageUrl||""} alt={labelForPose(scene.poseId)}/></button>)}</div><div className="carousel-reel-meta"><span>{scenes.length||6} scenes</span><i>•</i><span>~17 seconds</span><i>•</i><span>1080 × 1920</span></div></aside>
      ...
      <aside className="carousel-reel-right"><h2>From Every Angle<br/>to Every Audience</h2><p className="carousel-reel-right-intro">Turn your product into a story.</p><div className="carousel-reel-benefit"><span>◉</span><div><strong>Cinematic Camera Motion</strong><p>Slow push-ins, directional drifts and vertical garment scans create a premium fashion-film feel.</p></div></div><div className="carousel-reel-benefit"><span>✦</span><div><strong>Showcase Every Detail</strong><p>Reveal the garment from bottom to top and top to bottom so fabric, fit and finishing can be experienced.</p></div></div><div className="carousel-reel-benefit"><span>⌯</span><div><strong>Ready to Share</strong><p>Designed for social media, ads and marketplaces.</p></div></div><div className="carousel-reel-benefit"><span>♡</span><div><strong>Made for Fashion Brands</strong><p>Professional reels in seconds with AI.</p></div></div><div className="carousel-reel-signoff">Showcase Style<br/>Create Impact</div></aside>
    </section>

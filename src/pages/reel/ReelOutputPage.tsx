...
export default function ReelOutputPage() {
  ...
  return <div className="carousel-reel-page"><div className="carousel-reel-orb carousel-reel-orb-one"/><div className="carousel-reel-orb carousel-reel-orb-two"/><main className="carousel-reel-shell">
    ...
    <section className="carousel-reel-hero">
      <aside className="carousel-reel-left"><div className="carousel-reel-kicker">MAGICREEL AI STUDIO</div><h1>Carousel Reel</h1><p className="carousel-reel-description">A cinematic showcase of your collection with smooth transitions and elegant motion effects.</p><div className="carousel-reel-scenes">{scenes.slice(0,6).map((scene,index)=><button key={`${scene.poseId}-${index}`} className={`carousel-reel-scene ${selectedScene===index?"selected":""}`} onClick={()=>setSelectedScene(index)}><img src={scene.imageUrl||""} alt={labelForPose(scene.poseId)}/></button>)}</div><div className="carousel-reel-meta"><span>{scenes.length||6} scenes</span><i>•</i><span>~11 seconds</span><i>•</i><span>1080 × 1920</span></div></aside>
      <section className="carousel-reel-player-column"><div className="carousel-reel-player-frame"><video src={videoUrl} controls playsInline autoPlay muted poster={selectedImage}/><div className="carousel-reel-player-badge"><span>1080p</span><b>HD</b></div></div><div className="carousel-reel-arrows"><button onClick={()=>setSelectedScene((selectedScene-1+Math.max(scenes.length,1))%Math.max(scenes.length,1))}>‹</button><button onClick={()=>setSelectedScene((selectedScene+1)%Math.max(scenes.length,1))}>›</button></div></section>
      <aside className="carousel-reel-right"><h2>From Every Angle<br/>to Every Audience</h2><p className="carousel-reel-right-intro">Turn your product into a story.</p><div className="carousel-reel-benefit"><span>◉</span><div><strong>Cinematic Transitions</strong><p>Smooth zooms, pans and elegant moves for a premium look.</p></div></div><div className="carousel-reel-benefit"><span>✦</span><div><strong>Showcase Every Detail</strong><p>Highlight fit, fabric and design from multiple angles.</p></div></div><div className="carousel-reel-benefit"><span>⌯</span><div><strong>Ready to Share</strong><p>Perfect for social media, ads and marketplaces.</p></div></div><div className="carousel-reel-benefit"><span>♡</span><div><strong>Made for Fashion Brands</strong><p>Professional reels in seconds with AI.</p></div></div><div className="carousel-reel-signoff">Showcase Style<br/>Create Impact</div></aside>
    </section>
